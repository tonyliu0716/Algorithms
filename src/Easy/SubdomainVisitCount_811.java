...
public class SubdomainVisitCount_811 {
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for(int i = 0; i < cpdomains.length; i++) {
            String fullDomain = cpdomains[i];
            Integer number = Integer.parseInt(fullDomain.split(" ")[0]);
            // "google.mail.com"
            String domains = fullDomain.split(" ")[1];

            if(domains.indexOf(".") == -1) {
                // "9001 com"
                if(maps.get(domains) != null) {
                    maps.put(domains, maps.get(domains) + number);
                } else {
                    maps.put(domains, number);
                }

            } else {
                // "9001 google.mail.com"
    ...
    public static void main(String[] args) {
        ...
        List<String> lists = new SubdomainVisitCount_811().subdomainVisits(strings);
        for(String s : lists) {
